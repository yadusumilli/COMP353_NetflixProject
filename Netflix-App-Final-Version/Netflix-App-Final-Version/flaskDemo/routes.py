...
def Movies_TVShows():
    ...
    new_entertainment= entertainment(ShowID,Type,Title,DateAdded,ReleaseYear,Rating,Duration,Description)
    db.session.add(new_entertainment)
    db.session.commit()
    
    all = db.engine.execute("SELECT * FROM entertainment")
    
    result = db.engine.execute("SELECT COUNT(*) FROM entertainment WHERE Type='Movie'")
    print(list(result))
        
    if db.session.query(entertainment).filter(entertainment.ShowID == 2):
        # delete_ShowID = entertainment(ShowID = 2)
        db.session.query(entertainment).filter(entertainment.ShowID == 2).delete()
        db.session.commit()
    else:
        ShowID=2
        Type='Movie'
    ...
    result = db.engine.execute("UPDATE entertainment set ReleaseYear= 2020 WHERE ShowID=ANY(SELECT ShowID from entertainment WHERE Rating= 'TV-Y' AND ReleaseYear= 2019 AND Type = 'TV Show' AND Duration = '1 Season');")
